...
async function renderFantasiaAdmin(items=null){
  ...
  box.innerHTML=rows.map(i=>{const imgs=[i.frontImg,i.backImg,...(i.additionalImages||[])].filter(Boolean);const title=`🎭 ${i.country||"—"} — ${i.denomination||"—"}`;return `<article class="item market-admin-card">${imgs.length?`<button type="button" class="market-image-button" onclick='openCoinLightbox(${JSON.stringify(imgs)},0,${JSON.stringify(title)})'><img src="${imgs[0]}" alt="${esc(title)}"><span class="market-image-hint">↔ الوجه / الخلف • ⛶ تكبير</span></button>`:'<div class="market-image-button market-no-photo">لا توجد صورة محفوظة للمقتنى</div>'}<div class="market-admin-body"><h3>${esc(title)}</h3><p>${esc(i.year||"")} ${i.fantasiaIssuer?"— "+esc(i.fantasiaIssuer):""}</p><p class="muted">${esc(i.fantasiaNotes||"")}</p><div class="actions">${imgs.length?`<button class="ghost" onclick='openCoinLightbox(${JSON.stringify(imgs)},0,${JSON.stringify(title)})'>⛶ الصور (${imgs.length})</button>`:""}<button onclick="editItem('${i.id}')">تعديل</button>${archiveButton(i.id)}${adminMoveButtons(i,"warehouse")}<a class="public-link" href="/fantasia#item-${i.id}" target="_blank">عرض للزوار</a></div></div></article>`}).join("")||'<div class="empty">لا توجد مقتنيات فانتازيا حاليًا.</div>';
}
