...
let activeSession=null, room=null, isSeller=false, micOn=true, refreshTimer=null, countdownTimer=null;
...
async function refreshSession(){if(!activeSession)return;try{let rows=[];try{rows=(await json('/api/live-auctions/admin')).sessions||[]}catch{rows=(await json('/api/live-auctions/mine')).sessions||[]}const s=rows.find(x=>String(x.id)===String(activeSession.id));if(s){activeSession=s;renderCurrent(s);renderPrepared(s)}}catch{}}
