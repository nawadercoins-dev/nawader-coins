...
async function api(path,opt={}){let last=null;for(let attempt=0;attempt<3;attempt++){try{let r=await fetch(path,{...opt,headers:{'Content-Type':'application/json',...(opt.headers||{})},cache:'no-store',credentials:'same-origin'});let j={};try{j=await r.json()}catch{}if(!r.ok)throw new Error(j.error||('تعذر الاتصال ('+r.status+')'));return j}catch(e){last=e;if(attempt<2)await new Promise(ok=>setTimeout(ok,450*(attempt+1)))}}throw last||new Error('تعذر الاتصال')}
...
function setAuctionAccessMode(mode,p={}){
  const guest=mode==='guest', title=$('auctionAccessTitle'), help=$('auctionGuestHelp'), actions=$('auctionGuestActions'), notice=$('auctionGuestNotice'), box=$('participantStatusBox');
  if(actions)actions.hidden=!guest;
  if(help)help.hidden=!guest;
  if(notice)notice.hidden=!guest;
  if(box)box.hidden=guest;
  if(title)title.textContent=guest?'الدخول للمزايدة':'حساب المزاد';
  if(guest&&$('joinStatus'))$('joinStatus').textContent='تصفح المزادات متاح الآن، ويلزم تسجيل الدخول فقط عند تنفيذ المزايدة.';
}
function showParticipantStatus(p){if(!p)return;let s=(p.approvalStatus==='preliminary'?'final':(p.approvalStatus||(p.approved?'final':'new'))),ok=s==='final',labels={new:'طلب جديد',final:'توثيق كامل',suspended:'الحساب معلّق',stopped:'الحساب موقوف',cancelled:'الحساب ملغى'};setAuctionAccessMode('account',p);let box=$('participantStatusBox');if(box)box.hidden=false;if($('participantStatusTitle'))$('participantStatusTitle').textContent=(ok?'✅ ':'⏳ ')+(labels[s]||'حالة الحساب');if($('participantCode'))$('participantCode').textContent='رمز المشاركة: '+p.id;if($('joinStatus'))$('joinStatus').textContent=ok?'✅ جلسة الدخول فعالة ويمكنك المزايدة الآن':'الحساب مسجل الدخول، لكن المزايدة متوقفة حسب حالة الحساب.'}
async function checkParticipantStatus(){try{let r=await fetch('/api/participant/me',{cache:'no-store',credentials:'same-origin'}),d=await r.json().catch(()=>({}));if(!r.ok){pid='';auctionSessionValid=false;setAuctionAccessMode('guest');return false}let p=d.participant||{};pid=String(p.id||'');auctionSessionValid=!!pid;if(pid)localStorage.setItem('khazinaParticipantId',pid);showParticipantStatus(p);return auctionSessionValid}catch(e){if(!pid)setAuctionAccessMode('guest');if($('joinStatus'))$('joinStatus').textContent=pid?'جلسة الحساب محفوظة، وتعذر تحديث حالتها لحظيًا.':'تعذر التحقق من جلسة الدخول مؤقتًا.';return auctionSessionValid}}
$('register').onclick=()=>{location.href='/account?next='+encodeURIComponent('/auction'+STORE_Q)};function photo(src,id,groupId,index){if(!src)return'';let v='pv-'+id+'-'+Math.random().toString(36).slice(2,7);return `<div><div class="public-photo-tools" data-target="${v}"><button data-act="open" title="فتح الصورة بحجم كبير">⛶</button><button data-act="zin" title="تكبير">＋</button><button data-act="zout" title="تصغير">－</button><button data-act="rl" title="تدوير يسار">↺</button><button data-act="rr" title="تدوير يمين">↻</button><button data-act="reset" title="إعادة الضبط">إعادة</button></div><div class="public-photo" id="${v}"><img src="${esc(src)}" data-auction-group="${esc(groupId)}" data-auction-index="${index}" class="auction-expandable-image" title="اضغط لفتح الصورة بحجم كبير" style="cursor:zoom-in"></div></div>`}
...
function auctionToken(a){return a.map(i=>[i.id,i.auctionRound,i.auctionCurrentPrice,i.auctionEnd,i.reserveState,i.auctionEnded,i.auctionSold,i.updated].join(':')).join('|')}async function load(){try{let r=await api('/api/public/auctions'+STORE_Q),a=Array.isArray(r.items)?r.items:[],token=auctionToken(a);auctionAllItems=a;if(initialAuctionLoad||token!==auctionRenderToken){let y=window.scrollY;auctionRenderToken=token;renderAuctionDiscovery();if(!hashHandled&&location.hash){hashHandled=true;requestAnimationFrame(()=>{let el=document.querySelector(location.hash);if(el)el.scrollIntoView({block:'start'})})}else if(!initialAuctionLoad){requestAnimationFrame(()=>window.scrollTo({top:y,left:0,behavior:'auto'}))}}initialAuctionLoad=false;let note=$('auctionConnectionNote');if(note)note.remove()}catch(e){console.warn('تعذر تحديث المزادات مؤقتًا',e);if(initialAuctionLoad&&!auctionAllItems.length){$('publicAuctions').innerHTML='<div class="panel" id="auctionConnectionNote"><h2>تعذر تحميل المزادات الآن</h2><p>سيعيد النظام المحاولة تلقائيًا. جلسة الحساب لم تتأثر.</p><button type="button" onclick="load()">↻ إعادة المحاولة الآن</button></div>'}else{let box=$('publicAuctions');if(box&&!$('auctionConnectionNote'))box.insertAdjacentHTML('afterbegin','<div class="panel" id="auctionConnectionNote"><p>⚠ تعذر تحديث المزادات لحظيًا؛ المعروض الحالي محفوظ وسيعاد الاتصال تلقائيًا.</p></div>')}}}
...
window.bid=async id=>{let msg=$('msg-'+id),sw=$('swipe-'+id);let ok=auctionSessionValid||await checkParticipantStatus();if(!ok||!pid){msg.textContent='🔐 يلزم تسجيل الدخول للمزايدة. المزاد نفسه ما زال نشطًا.';resetSwipe(id);setTimeout(()=>{location.href='/account?next='+encodeURIComponent('/auction'+STORE_Q+'#'+id)},600);return}try{let amount=Number($('amt-'+id).value||0);if(amount<=0)throw new Error('اكتب مبلغ المزايدة أولًا');let r=await api('/api/bid',{method:'POST',body:JSON.stringify({itemId:id,participantId:pid,amount})});msg.textContent='✅ تم قبول مزايدتك: '+money(r.current);await load()}catch(e){msg.textContent='⚠️ '+e.message;resetSwipe(id)}};

// V4.0.20: أزيل التفاوض من واجهة المزاد العامة.
function paintAuctionClocks(){document.querySelectorAll('.auction-clock').forEach(x=>{x.textContent=clock(x.dataset.end);let d=new Date(x.dataset.end).getTime()-Date.now();x.classList.remove('clock-hour','clock-ten','clock-minute');if(d>0&&d<=60000)x.classList.add('clock-minute');else if(d>0&&d<=600000)x.classList.add('clock-ten');else if(d>0&&d<=3600000)x.classList.add('clock-hour')})}setInterval(paintAuctionClocks,1000);paintAuctionClocks();setInterval(load,8000);setInterval(checkParticipantStatus,3000);function applyStoreBranding(){document.title='المزادات | '+STORE_LABEL;document.querySelectorAll('.brand h1').forEach(x=>x.textContent=STORE_LABEL);document.querySelectorAll('a.home').forEach(x=>x.href=STORE_HOME);document.querySelectorAll('a[href^="/market"]').forEach(x=>x.href='/market'+STORE_Q);document.querySelectorAll('a[href^="/account"]').forEach(x=>x.href='/account'+STORE_Q);document.querySelectorAll('a[href^="/notifications"]').forEach(x=>x.href='/notifications'+STORE_Q);let special=document.querySelector('a[href^="/special-numbers"]');if(special&&STORE==='collectibles'){special.href='/fantasia';special.textContent='🎭 الفنتازيا'}let badge=document.querySelector('.hero-badge');if(badge)badge.textContent='◆ '+STORE_LABEL;let p=document.querySelector('.brand p');if(p)p.textContent='المزادات — '+(STORE==='collectibles'?'تحف وفنتازيا ومقتنيات':'عملات أصلية ورسمية');let help=$('auctionGuestHelp');if(help)help.textContent='التصفح متاح للجميع. عند الرغبة في المزايدة استخدم حسابك الموثق في '+STORE_LABEL+'.';}
applyStoreBranding();loadSettings().then(load);checkParticipantStatus();
