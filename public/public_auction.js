...
$('register').onclick=()=>{location.href='/account?next='+encodeURIComponent('/auction')};function photo(src,id,groupId,index){if(!src)return'';let v='pv-'+id+'-'+Math.random().toString(36).slice(2,7);return `<div><div class="public-photo-tools" data-target="${v}"><button data-act="open" title="فتح الصورة بحجم كبير">⛶</button><button data-act="zin" title="تكبير">＋</button><button data-act="zout" title="تصغير">－</button><button data-act="rl" title="تدوير يسار">↺</button><button data-act="rr" title="تدوير يمين">↻</button><button data-act="reset" title="إعادة الضبط">إعادة</button></div><div class="public-photo" id="${v}"><img src="${esc(src)}" data-auction-group="${esc(groupId)}" data-auction-index="${index}" class="auction-expandable-image" title="اضغط لفتح الصورة بحجم كبير" style="cursor:zoom-in"></div></div>`}
