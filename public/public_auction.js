...
function showParticipantStatus(p){if(!p)return;let box=$('participantStatusBox');if(box)box.hidden=false;let s=(p.approvalStatus==='preliminary'?'final':(p.approvalStatus||(p.approved?'final':'new'))),ok=s==='final',labels={new:'طلب جديد',final:'توثيق كامل',suspended:'الحساب معلّق',stopped:'الحساب موقوف',cancelled:'الحساب ملغى'};if($('participantStatusTitle'))$('participantStatusTitle').textContent=(ok?'✅ ':'⏳ ')+(labels[s]||'حالة الحساب');if($('participantCode'))$('participantCode').textContent='رمز المشاركة: '+p.id;if($('joinStatus'))$('joinStatus').textContent=ok?'✅ جلسة الدخول فعالة ويمكنك المزايدة الآن':'المزاد نشط، لكن عمليات الحساب متوقفة حسب حالة الحساب.'}
async function checkParticipantStatus(){try{let r=await fetch('/api/participant/me',{cache:'no-store'}),d=await r.json().catch(()=>({}));if(!r.ok){pid='';auctionSessionValid=false;if($('joinStatus'))$('joinStatus').textContent='المزاد نشط. يلزم تسجيل الدخول فقط عند تنفيذ المزايدة.';return false}let p=d.participant||{};pid=String(p.id||'');auctionSessionValid=!!pid;if(pid)localStorage.setItem('khazinaParticipantId',pid);showParticipantStatus(p);return auctionSessionValid}catch(e){if($('joinStatus'))$('joinStatus').textContent='تعذر التحقق من جلسة الدخول مؤقتًا؛ المزاد ما زال نشطًا.';return auctionSessionValid}}
